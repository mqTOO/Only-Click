...
function createBubble() {
    ...
    if (isBonus) {
        ...
        bubble.addEventListener('click', () => {
            ...
            localStorage.setItem('totalBubbles', totalBubbles); // Сохраняем собранные пузыри
            scoreElement.textContent = `Счет: ${score}`;
            totalBubblesElement.textContent = `Собрано пузырей: ${totalBubbles}`; // Обновляем отображение собранных пузырей
...
document.getElementById('back-to-menu-btn').addEventListener('click', () => {
    ...
    document.getElementById('menu-background').style.display = 'block'; // Показываем фон главного меню
});
...
document.getElementById('resume-btn').addEventListener('click', () => {
    ...
    document.getElementById('game').style.display = 'block'; // Показываем игровой экран
    bubbleInterval = setInterval(() => {
        createBubble();
        updateLevel();
    }, levelInterval); // Запускаем создание пузырей
    timerInterval = setInterval(() => {
        gameTime--;
        timerElement.textContent = `Осталось: ${gameTime} секунд`;
        if (gameTime <= 0) {
...
// Обработчики событий для кнопки "Выход в меню" на экране паузы
document.getElementById('exit-to-menu-btn').addEventListener('click', () => {
    clearInterval(bubbleInterval); // Останавливаем создание пузырей
    clearInterval(timerInterval); // Останавливаем таймер

    // Скрываем экран игры и экран паузы
    document.getElementById('game').style.display = 'none'; // Скрываем экран игры
    document.getElementById('pause-screen').style.display = 'none'; // Скрываем экран паузы

    // Показываем экран главного меню
